...
SELECT 
  SUM(CASE 
        WHEN masa_berlaku IS NOT NULL 
        AND masa_berlaku != '0000-00-00'
        AND DATEDIFF(masa_berlaku, CURDATE()) < 0 
  ...
  SUM(CASE 
        WHEN masa_berlaku IS NOT NULL 
        AND masa_berlaku != '0000-00-00'
        AND DATEDIFF(masa_berlaku, CURDATE()) > 30 
      THEN 1 ELSE 0 END) as aman

FROM pegawai_pns
";

$resultNotif = $koneksi->query($queryNotif);
$dataNotif = $resultNotif->fetch_assoc();

$expired = $dataNotif['expired'] ?? 0;
$warning = $dataNotif['warning'] ?? 0;
$aman    = $dataNotif['aman'] ?? 0;
?>
      ...
      <div class="jenis-header">
        <div class="jenis-title-wrap">
          ...
          <div>
            ...
            <span>Kelola data pegawai negeri sipil secara lengkap</span>
          </div>
        </div>
      </div>
      <div class="action-buttons-modern">
        <a href="addPNS.php" class="btn-modern primary">
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr class="data-row">
                      ...
                      <td class="col-alamat" title="<?= $row['alamat_rumah']; ?>"><?= $row['alamat_rumah']; ?></td>
                      <td class="col-hp"><?= $row['no_telpon']; ?></td>
                      ...
                      <td class="col-aksi text-center">
                        <div class="action-group">
                          ...
                          <a href="editPNS.php?id=<?= $row['id']; ?>"
                            class="action-btn edit"
                            title="Edit">
                            <i class="fas fa-edit"></i>
                          </a>

                          <a href="../../backend/PNS/delete.php?id=<?= $row['id']; ?>"
